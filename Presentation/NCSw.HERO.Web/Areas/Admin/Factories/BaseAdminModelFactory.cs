...
using NCSw.HERO.Core.Domain.Logging;
using NCSw.HERO.Core.Domain.Orders;
using NCSw.HERO.Core.Domain.Payments;
...
using NCSw.HERO.Services.Directory;
...
using NCSw.HERO.Services.Shipping.Date;
using NCSw.HERO.Services.Stores;
using NCSw.HERO.Services.Tax;
using NCSw.HERO.Services.Topics;
using NCSw.HERO.Services.Vendors;
using NCSw.HERO.Web.Areas.Admin.Helpers;
...
namespace NCSw.HERO.Web.Areas.Admin.Factories
{
    ...
    public partial class BaseAdminModelFactory : IBaseAdminModelFactory
    {
        ...
        #region HERO Methods 
        public void PrepareDepartmentTemplates(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));
            ...
            {
                Text = _localizationService.GetResource("Admin.Department.Select2.SelectRoom"),
                Value = "0"
            });
            //Get follow Path
        ...
        public void PrepareServiceTemplates(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available category templates
            var availableDepTemplates = _serviceService.GetAllItem(languageId: 0, showHidden: false);
            items.Add(new SelectListItem()
            {
                Text = _localizationService.GetResource("Admin.Department.Select2.SelectService"),
                Value = "0"
            });
            //Get follow Path
            foreach (var template in availableDepTemplates)
        ...
        public void PrepareDoctorTemplates(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available category templates
            var availableDepTemplates = _doctorService.GetAll(languageId: 0, showHidden: false);
            items.Add(new SelectListItem()
            {
                Text = _localizationService.GetResource("Admin.Department.Select2.SelectDoctor"),
                Value = "0"
            });
            //Get follow Path
            foreach (var template in availableDepTemplates)
            {
                items.Add(new SelectListItem { Value = template.Id.ToString(), Text = template.FullName });
            }
            //insert special item for the default value
            PrepareDefaultItem(items, withSpecialDefaultItem, defaultItemText);
        }
